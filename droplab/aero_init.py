...
def model_init(dt_widget, nt_widget, Condensation_widget, Collision_widget, n_particles_widget, T_widget, P_widget, RH_widget, w_widget, z_widget, max_z_widget, mode_aero_init_widget, gridwidget, ascending_mode_widget, mode_displaytype_widget, switch_kappa_koehler):
    ...
    # All-droplet (haze included) weighted mean/std radius, complementing the cloud-only rc_ arrays
    ...
    # Id-indexed cloud+rain radii per timestep (0 for haze)
    particles_c_array = np.zeros((nt+1, len(particles_list)))
...
def r_equi_arr(S, T, r_aerosol, rho_aero_, switch_kappa_koehler, kappa):
    """Vectorized (numba, parallel) r_equi over per-droplet arrays S, T, r_aerosol, kappa.
    BIT-IDENTICAL math to the scalar r_equi above: same fixed-point iteration, same
    convergence test, and `** 3.0` (not `** 3`) so numba uses the same C pow() rounding
    as CPython's float power. Each droplet writes only its own slot -> prange-safe and
    thread-order independent. This is the droplet-initialization hot path (~10 Newton
    iterations x n_super pure-Python calls otherwise)."""
    n = S.shape[0]
    out = np.empty(n)
    for i in _prange(n):
        S_internal = min(S[i], -0.0001)
        afactor = 2.0 * sigma_air_liq(T[i]) / (rho_liq * rv * T[i])
        if switch_kappa_koehler:
            bfactor = kappa[i]
        else:
            bfactor = vanthoff_aero * rho_aero_ * molecular_weight_water / (rho_liq * molecular_weight_aero)
        r0 = 1.0
        r = 1.0e-6
        while abs((r - r0) / r0) > 1.0e-20:
            r0 = r
            r = ((bfactor * r_aerosol[i] ** 3.0) / (afactor / r - S_internal)) ** (1.0 / 3.0)
        out[i] = r
    return out