...
Drop-in array version of OUR validated object collision in droplab/collision.py.
...
The object reference in droplab/collision.py stays the source of truth; this module
...
from droplab.collision import E_H80, ws_drops_beard
from droplab.condensation import sigma_air_liq
...
import droplab.collision as _coll
...
@njit(cache=True)
def _sm64_next(s):
    """splitmix64 step (Steele et al. 2014): counter-based, high-quality 64-bit PRNG.
    Pure function of the state -> thread- and schedule-independent; uint64 wraps like C."""
...
@njit(cache=True)
def _sm64_uniform(s):
    """One U(0,1) double from the splitmix64 stream (53 mantissa bits)."""
    s, z = _sm64_next(s)
    return s, float(z >> np.uint64(11)) * (1.0 / 9007199254740992.0)
...
def _collision_kernel(M, A, Ns, kappa, idx, half, nptcl,
                      ...
                      switch_turb_kernel, epsilon_turb, V_parcel,
                      phase, c, rimed,
                      use_global_rng=True, rng_state=np.uint64(0)):
    ...
    from droplab/collision.py EXACTLY, on arrays. Pairs idx[k] with idx[k+half].
    `V_parcel` is the COLLISION volume (the volume the `nptcl` super-droplets
    occupy): for a 1D parcel it is PARCEL_AIR_MASS/rho; for a 2D grid cell it MUST
    be the cell volume dx*dz*depth (NOT the parcel volume, or the rate is wrong by
    V_cell/V_parcel). rho_parcel is still used for terminal velocity / the kernel.
    Returns (n_collisions, rng_state).

    RNG: with use_global_rng=True (the default) every accept/reject draw comes from
    numba's global sequential stream — the golden bit-identical path. With False the
    draws come from the explicit counter-based splitmix64 `rng_state` (thread-safe,
    schedule-independent), which is what the opt-in parallel per-cell collision uses.
    """
    n_coll = 0
    ...
    for k in range(half):
        ...
        if use_global_rng:
            ...
            rng_state, x_rand = _sm64_uniform(rng_state)
        ...
        if p_crit > x_rand:
            ...
            # ---- update (phase-aware: liquid coalescence / ice aggregation / riming) ----
            if phase[i] == 0 and phase[j] == 0:
                # liquid-liquid coalescence (UNCHANGED golden path)
                if Ai == Aj:
                    _same_weights_update(M, A, Ns, kappa, i, j)
                else:
                    _liquid_update_collection(M, A, Ns, kappa, i, j, p_int)
            elif phase[i] == 1 and phase[j] == 1:
                # ice-ice aggregation: same mass merge, result stays ice
                if Ai == Aj:
                    _same_weights_update(M, A, Ns, kappa, i, j)
                else:
                    _liquid_update_collection(M, A, Ns, kappa, i, j, p_int)
            else:
                # mixed ice+liquid: riming -> result ice, frozen liquid mass -> rimed[c]
                rimed[c] += _rime_update(M, A, Ns, kappa, phase, i, j, p_int)

            n_coll += 1

    return n_coll, rng_state
...
@njit(cache=True)
def _rime_update(M, A, Ns, kappa, phase, i, j, p_int):
    """Mixed-phase collision (riming): an ice and a liquid super-droplet collide. The
    smaller-A super-droplet gains mass exactly as in coalescence, but the result is ICE,
    and the latent heat of fusion is released for the LIQUID mass that freezes. Returns
    that rimed (frozen) liquid mass; total water mass is conserved. MVP: collision uses the
    same gravitational kernel as liquid (liquid-equivalent radius), not an ice habit/Boehm
    efficiency (that is the deferred ice-habits tier)."""
    if A[i] < A[j]:
        a = i; b = j
    else:
        a = j; b = i
    ...
    xs_int = Ns[b] / A[b]
    A_a = A[a]
    transferred = A_a * x_int * p_int
    rimed = 0.0
    if phase[a] == 0:
        rimed += M[a]                              # gainer was liquid -> its mass freezes
    if phase[b] == 0:
        rimed += transferred                       # collected liquid freezes onto ice
    M[a] = M[a] + transferred                      # same mass merge as coalescence
    Ns[a] = Ns[a] + A_a * xs_int * p_int
    A[b] = A[b] - A_a * p_int
    M[b] = x_int * A[b]
    Ns[b] = xs_int * A[b]
    phase[a] = 1                                   # the combined super-droplet is ice
    return rimed
...
def collide_soa(M, A, Ns, kappa, dt, rho_parcel, p_env, T_parcel,
                ...
                switch_turb_kernel=False, epsilon_turb=0.0, V_parcel=None):
    ...
    Parameters mirror determine_collision()/collection() in droplab/collision.py.
    ...
    # default (1D parcel) collision volume; 2D passes the cell volume explicitly
    Vp = PARCEL_AIR_MASS / rho_parcel if V_parcel is None else float(V_parcel)
    _phase0 = np.zeros(n, dtype=np.int8)               # 1D parcel: all liquid (no ice path)
    _rimed0 = np.zeros(1)
    _collision_kernel(M, A, Ns, kappa, idx, half, n,
                      ...
                      bool(switch_turb_kernel), float(epsilon_turb), Vp,
                      _phase0, 0, _rimed0)
