#!/usr/bin/env python3
"""Command-line runner for droplab, driven by a YAML config (input.yaml).
...
def run_kwargs(cfg, mixing, collect, seed=0):
    ...
    return dict(seed=seed, n_ptcl=a["n_ptcl"], nt=p["nt"], dt=p["dt"],
                ...
                switch_TICE=phys["turbulent"], eps=phys["epsilon"],
...
def main():
    ap = argparse.ArgumentParser(description="Run droplab from a YAML config.")
