# Collision Algorithm Comparison: droplab vs Fortran Reference Codes
...
|------|--------|----------------|-------|------------|
| **droplab** (`collision.py`) | LSM | Yes (Shima 2009) | Yes | O(N) |
...
| Component | droplab | SAM | Fortran Box |
|-----------|-------|-----|-------------|
...
**droplab** (`collision.py:220-231`) - AFTER fix:
```python
if p_crit <= 1.0:
    p_crit = 1
else:
...
**droplab** (`collision.py:78-131`) - `liquid_update_collection()`:
```python
...
droplab and the Fortran box model both compute:
```
...
SAM does NOT use E_S09 in its active code (the kernel line at `micro_coll.f90:690` omits it). This means SAM should produce slightly more collisions than droplab for the same setup (since E_S09 <= 1.0 always reduces the kernel).

### 3.9 Kappa Update (Volume-Weighted Mean)

Only droplab tracks kappa after collision:
```python
...
| Aspect | LSM (droplab, SAM) | Full Enumeration (Box Model) |
|--------|-------------------|------------------------------|
...
**Before the fix**, droplab's `determine_collision()` returned only `check_final` and `check_collection` (booleans). When p_crit exceeded 1.0, a collision was registered, but only a single collision was applied to the mass transfer. This meant:
...
### 5.2 Before Multi-Collision Fix (droplab, p_crit=1 always)

| Metric | droplab (old) | Fortran Box | Ratio |
|--------|-------------|-------------|-------|
...
The ~10x discrepancy in Nr was the primary issue: droplab produced far too many rain drops because each collision event only moved p_crit=1 worth of mass, fragmenting the rain population.

### 5.3 After Multi-Collision Fix (droplab, p_crit=NINT(p_crit))

| Metric | droplab (new) | Fortran Box | Ratio |
|--------|-------------|-------------|-------|
...
The remaining factor-of-3 difference in peak Nr between droplab and the Fortran box model is expected due to:
...
3. **E_S09 in droplab**: droplab includes coalescence efficiency E_S09 which slightly reduces the kernel. SAM does not use E_S09, so SAM would produce slightly more efficient collection. The Fortran box model does use E_S09.

### 5.5 Ensemble droplab Results (2 members, seeds 100 and 200)
...
2. **droplab now agrees with SAM's implementation**. Both use identical LSM pairing, p_crit upscaling, multi-collision with NINT rounding, and the p_crit limiter.
...
4. **Performance**: droplab with multi-collision is ~6x faster than the Fortran box model (251s vs 1500s for 10k particles), while producing scientifically consistent results.
...
|------|----------|-------------|
| `collision.py` | `droplab/collision.py` | droplab collision module (this project) |
...
*Document generated 2026-02-10. Based on analysis of droplab commit f3820d6 + multi-collision fix.*
